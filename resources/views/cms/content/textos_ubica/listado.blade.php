        ...
        <tbody>
          @foreach ($datos as $texto)
            <tr>
              ...
              <td>
                <a class="btn btn-primary" href="{{ url('cms/textos_ubica/' . $ubicacion . '/' . $texto->id . '/edit') }}"><i
                  class="bx bx-edit-alt me-1"></i> Editar</a>
                {{-- <button type="button" class="btn btn-primary btn-icon" data-bs-toggle="dropdown"><i
                    class="bx bx-dots-vertical-rounded"></i></button>
                <div class="dropdown-menu">
                  <a class="dropdown-item" href="{{ url('cms/textos_ubica/' . $ubicacion . '/' . $texto->id . '/edit') }}"><i
                      class="bx bx-edit-alt me-1"></i> Editar</a>
                  <form action="{{ route('cms.textos_ubica.destroy', ['ubicacion' => $ubicacion, 'texto' => $texto->id]) }}" method="POST"
